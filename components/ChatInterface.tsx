...
const QUICK_TOPICS = [
  ...
  { label: 'Code de la route', question: 'Quelles sont les sanctions pour excès de vitesse au Maroc ?' },
]
...
export default function ChatInterface({ userEmail }: { userEmail: string }) {
  ...
  async function sendMessage(content: string) {
    ...
    try {
      ...
      setMessages((prev) => {
        const updated = [...prev]
        updated[updated.length - 1] = { role: 'assistant', content: 'Une erreur est survenue. Veuillez réessayer.' }
        return updated
      })
    } finally {
  ...
  return (
    <div className="flex-1 bg-white border border-gold/20 rounded-lg flex flex-col overflow-hidden min-h-[500px]">
      <div className="flex-1 overflow-y-auto p-6 space-y-5">
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center py-8">
            ...
            <h2 className="font-playfair text-xl font-bold text-forest mb-2">Bienvenue, {username}</h2>
            <p className="font-ibm text-sm text-ink/60 max-w-sm mb-8">
              Posez votre question en droit marocain. Je cite les articles de loi pertinents.
            </p>
            <div className="grid grid-cols-2 gap-3 w-full max-w-md">
              {QUICK_TOPICS.map((topic) => (
                <button key={topic.label} onClick={() => sendMessage(topic.question)}
          ...
          <>
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                {msg.role === 'user' && (
                  <div className="w-7 h-7 bg-gold rounded-full flex items-center justify-center ml-3 mt-1 flex-shrink-0 text-xs text-white font-bold uppercase">{username[0]}</div>
                )}
              </div>
            ))}
      ...
      <div className="border-t border-gold/20 p-4">
        <form onSubmit={(e: FormEvent) => { e.preventDefault(); sendMessage(input) }} className="flex gap-3">
          <input ref={inputRef} type="text" value={input} onChange={(e) => setInput(e.target.value)}
            placeholder="Posez votre question juridique…" disabled={isStreaming}
